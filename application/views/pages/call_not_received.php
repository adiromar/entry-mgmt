...
<div class="container-fluid">
	<div class="card card_prof">
		<h4 class="mt-3 mb-4" style="text-align: center;">Call Not Received/Later On Entries </h4>

<table class="table table-bordered table-striped resp">
			<thead class="heading_mob thead-dark" id="heading_mob" style="background-color: #dee2e6">
				<tr>
					<th>S.N.</th>
					<th>Mobile Number</th>
					<th>Other Info</th>
			...
			<tbody class="tbl_content">
				...
				foreach ($later as $val) {
					...
					echo '<tr>';
						...
						if ($val['status'] == 'Later On'){
							echo '<td><button class="btn btn-primary">'.$val['status'].'</button></td>';
						}else if ($val['status'] == 'Call Not Received'){
							echo '<td><button class="btn btn-warning">'.$val['status'].'</button></td>';
						}else if ($val['status'] == 'Inquiry Only'){
							echo '<td><button class="btn btn-secondary">'.$val['status'].'</button></td>';
						}else{
							echo '<td><button class="btn btn-light">Pending</button></td>';
						}
